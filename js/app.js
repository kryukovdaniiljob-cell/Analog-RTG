...
fetch('data.json?v=1').then(r => r.json()).then(d => {
  ...
  заполнить($('f-напр'), D.н.map((v, i) => [i, v]));
...
function подходит(r, уровень) {
  const н = $('f-напр').value, г = $('f-груп').value, к = $('f-кат').value;
  if (уровень > 0 && н !== '' && r[3] != н) return false;
  if (уровень > 1 && г !== '' && r[4] != г) return false;
  if (уровень > 2 && к !== '' && r[5] != к) return false;
  return true;
}
...
const группыДля = () => собрать1(4, D.г, 1);
const категорииДля = () => собрать1(5, D.к, 2);
const брендыДля = () => собрать1(2, D.б, 3);

function каскад(с_уровня) {
  if (с_уровня <= 1) {
    заполнить($('f-груп'), группыДля());
    if ($('f-напр').value === '') { $('f-груп').value = ''; }
  }
  if (с_уровня <= 2) заполнить($('f-кат'), категорииДля());
  заполнить($('f-бренд'), брендыДля());
}
...
function отбор() {
  ...
  const н = $('f-напр').value, г = $('f-груп').value,
        к = $('f-кат').value, б = $('f-бренд').value, з = $('f-зам').value;
  найдено = P.filter(r => {
    for (const w of сл) if (!r[9].includes(w)) return false;
...
function крошки() {
  const ч = [];
  const пары = [['f-напр', D.н, 'Направление'], ['f-груп', D.г, 'Группа'],
...
$('f-напр').addEventListener('change', () => { каскад(1); отбор(); });
$('f-груп').addEventListener('change', () => { каскад(2); отбор(); });
$('f-кат').addEventListener('change', () => { каскад(3); отбор(); });
$('f-бренд').addEventListener('change', отбор);
$('f-зам').addEventListener('change', отбор);
$('сброс').addEventListener('click', () => {
  ['q', 'f-напр', 'f-груп', 'f-кат', 'f-бренд', 'f-зам'].forEach(id => $(id).value = '');
  открыта = -1; $('карточка').innerHTML = '';
  каскад(1); отбор();
});
